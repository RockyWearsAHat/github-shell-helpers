...
function createHandler(deps) {
  const { archiveRoot, workspaceRoots = [] } = deps;
  ...
  function searchChatHistory(args) {
    ...
    for (const session of Object.values(manifest.sessions)) {
      ...
      for (const chunk of session.chunks || []) {
        // Date range pre-check — skip chunks outside requested window
        ...
        // Bloom filter pre-check — skip chunks that definitely don't match
        if (
        ...
        const text = readCompressedUtf8(chunk.textPath);
        if (!text) continue;
        ...
        const snippet = buildSnippet(text, query, queryTokens);
        results.push({
    ...
    return {
      ...
      sessionsSearched: Object.keys(manifest.sessions).length,
    };
  }
  ...
  function formatSearchResults(result) {
    ...
    for (const r of result.results) {
      ...
      lines.push(
        `--- [${r.compressedLevel}] ${r.title} (${r.sessionId.slice(0, 8)}...) ${dateStr} ---`,
      );
