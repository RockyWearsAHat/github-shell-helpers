  ...
  async function searchGoogleHeadless(url) {
    ...
    for (let attempt = 0; attempt < RETRY_MAX_ATTEMPTS; attempt++) {
      ...
      try {
        ...
        if (!outcome.challenge && outcome.results.length === 0) {
          process.stderr.write(
            `[git-research-mcp] Headless got 0 results, no challenge flag. URL=${outcome.pageUrl || page.url()} title=${JSON.stringify(outcome.pageTitle || "")} body=${JSON.stringify((outcome.bodyText || "").slice(0, 300))} status=${response ? response.status() : "unknown"}\n`,
          );
        }
