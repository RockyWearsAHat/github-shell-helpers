...
module.exports = function createGoogleHeadless(deps) {
  ...
  async function getBrowserUserDataDir() {
    ...
    // Clean up stale profile locks from previous sessions.
    // If Chrome crashed or the MCP server was killed, the SingletonLock symlink
    // persists and prevents new headless launches from using the profile.
    try {
  ...
  async function runInteractiveGoogleBrowser(task) {
    ...
    // Use a separate temp directory for the interactive browser so it never
    // conflicts with the headless browser's profile lock on userDataDir.
    const interactiveDataDir = path.join(
      os.tmpdir(),
      `gsh-captcha-chrome-${Date.now()}`,
    );
    ...
    try {
      ...
      if (shouldCloseBrowser) {
        ...
        fs.rm(interactiveDataDir, { recursive: true, force: true }).catch(
          () => {},
        );
      }
    }
    ...
    async function transferCookiesToHeadless(page) {
      try {
        ...
        if (googleCookies.length > 0) {
          ...
          await fs.writeFile(cookiePath, JSON.stringify(googleCookies), "utf8");
          process.stderr.write(
