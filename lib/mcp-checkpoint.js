  ...
  inputSchema: {
    ...
    properties: {
      ...
      model: {
        type: "string",
        description:
          "Model to use for AI commit message generation. Accepts a model id (e.g. 'gpt-5.4-mini'), display name (e.g. 'Claude Haiku 4.5'), or shorthand (e.g. 'haiku'). When omitted, the cheapest available model is selected automatically.",
      },
    },
    required: [],
  },
...
async function generateAiCommitMessage(cwd, extraContext, preferredModel) {
...
${detailedRecent}`;
  ...
  const aiCmd =
    ...
    (() => {
      // Resolve the model: user preference → auto-detect cheapest available.
      const models = loadAvailableModels();
      let modelId;
      if (preferredModel) {
        modelId =
          resolveModelId(preferredModel, models) || preferredModel.trim();
      } else {
        modelId = detectCheapModel(models);
      }
      return `copilot -s --model ${modelId} --deny-tool write --deny-tool shell -p "$GIT_UPLOAD_AI_PROMPT"`;
    })();
...
async function handleCheckpoint(args) {
  ...
  const stat = await execGit(["show", "--stat", "--format=", "HEAD"], cwd);
