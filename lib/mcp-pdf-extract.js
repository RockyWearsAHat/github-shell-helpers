...
function createPdfParser(pdfBuffer, pdfParseModule = getPDFParse()) {
  const classExport =
    pdfParseModule && typeof pdfParseModule.PDFParse === "function"
      ...
      : null;
  const legacyExport = typeof pdfParseModule === "function" ? pdfParseModule : null;
...
async function processPdf(url, opts) {
  ...
  if (parser) {
    ...
    totalChars = pageArray.reduce((sum, page) => sum + (page.text || "").length, 0);
    ...
    const legacyText = typeof legacyResult?.text === "string" ? legacyResult.text.trim() : "";
    text = legacyText;
