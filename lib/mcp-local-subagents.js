...
//
//   * `build_workspace_tool` — convenience wrapper around `system_execute`
//     for short "build this tool, verify it, then use it" requests. The
//     local agent creates a workspace script/command, runs a validation loop,
//     refines until the requested behavior is met, and can optionally use the
//     verified tool for a follow-up task.
//
...
const BUILD_WORKSPACE_TOOL = {
  name: "build_workspace_tool",
  description:
    "Ask Copilot to implement a workspace tool and register it as a callable MCP tool. Copilot writes the implementation and calls register_workspace_tool, making it immediately available via tools/call on this server. Use this when the user says 'build a tool', 'save this as a tool', or 'save it as a tool'.",
  inputSchema: {
    type: "object",
    properties: {
      tool_request: {
        type: "string",
        description:
          "What the tool should do. Be outcome-focused, e.g. 'A tool that returns the current git branch as plain text.'",
      },
      tool_name_hint: {
        type: "string",
        description:
          "Optional preferred name for the tool in snake_case, e.g. 'get_git_branch'. If omitted, Copilot chooses.",
      },
      language: {
        type: "string",
...
function resolveBrowserUserDataDir() {
  const env = process.env.GSH_LOCAL_SUBAGENT_BROWSER_USER_DATA_DIR;
  if (env && env.trim()) return env.trim();
  return path.join(os.homedir(), ".cache", "gsh", "playwright-profile");
}

// Path to the cancel/redirect control file. The extension writes here when
// the user presses Cmd+Shift+Esc; system_execute polls between iterations.
function systemExecuteControlPath() {
  return path.join(os.homedir(), ".cache", "gsh", "system-execute-control.json");
}

function systemExecuteActiveDir() {
  return path.join(os.homedir(), ".cache", "gsh", "system-execute-active");
}
...
function buildWorkspaceToolTask(args, workspaceRoot) {
  const toolRequest = String(args?.tool_request || "").trim();
  if (!toolRequest) return "";

  const successCriteria = String(args?.success_criteria || "").trim();
  const verificationCommand = String(args?.verification_command || "").trim();
  const followUpTask = String(args?.follow_up_task || "").trim();
  const toolPathHint = String(args?.tool_path_hint || "").trim();

  const sections = [
    "You are building or refining a reusable workspace tool before the main task continues.",
    `Workspace root: ${workspaceRoot}`,
    "Move quickly. This wrapper exists so the orchestrator can say 'build the tool, then do the real task' without spending a long prompt on setup.",
    `Tool request:\n${toolRequest}`,
    toolPathHint
      ? `Preferred path or entry point:\n${toolPathHint}`
      : null,
    successCriteria
      ? `Desired outcome:\n${successCriteria}`
      : "Desired outcome:\nInfer the narrowest practical success criteria from the request and keep the tool focused on that.",
    verificationCommand
      ? `Verification command:\n${verificationCommand}`
      : "Verification command:\nChoose the cheapest reliable executable check you can derive from the request. Prefer direct script invocation or a narrow targeted test over broad workspace-wide checks.",
    [
      "Required loop:",
      "1. Design the smallest viable interface for the tool.",
      "2. Create or edit only the workspace files needed for it.",
      "3. Run the verification command or derived check.",
      "4. If the result is wrong, refine the tool and retry.",
      "5. Stay in this loop until the tool matches the desired outcome or you hit a concrete blocker.",
      "6. Do not stop after the first draft just because the file exists.",
    ].join("\n"),
    [
      "Implementation rules:",
      "- Keep the tool reusable and easy to invoke later.",
      "- Prefer minimal files and minimal dependencies.",
      "- Do not rewrite unrelated code.",
      "- Add brief usage text only when it materially improves discoverability.",
    ].join("\n"),
    followUpTask
      ? `After verification:\nUse the verified tool to complete this follow-up task:\n${followUpTask}`
      : "After verification:\nSummarize the final tool path, invocation, and the exact behavior you verified.",
    "Return only after the tool is verified and any follow-up work is done. If blocked, name the blocker precisely and include the last verification result.",
  ];
...
let _globalNodeModulesPath = null;
function getGlobalNodeModulesPath() {
  if (_globalNodeModulesPath !== null) return _globalNodeModulesPath;
  try {
    const { execSync } = require("child_process");
    _globalNodeModulesPath = execSync("npm root -g", {
      encoding: "utf8",
      stdio: ["ignore", "pipe", "ignore"],
      timeout: 4000,
    }).trim();
  } catch {
    _globalNodeModulesPath = "";
  }
  return _globalNodeModulesPath;
}

function tryRequireFrom(name, fromGlobal) {
  try {
    if (fromGlobal) {
      const globalRoot = getGlobalNodeModulesPath();
      if (!globalRoot) return null;
      const resolved = require.resolve(name, { paths: [globalRoot] });
...
function loadPlaywright() {
  ...
  for (const name of ["playwright", "playwright-core"]) {
    for (const fromGlobal of [false, true]) {
      const mod = tryRequireFrom(name, fromGlobal);
      if (mod) {
        _playwrightModule = mod;
        return mod;
      }
    }
  }
...
function buildSystemAgentTools({
  ...
  async function ensureBrowser() {
    ...
    const userDataDir = browserState.userDataDir;
    const launchOpts = {
    ...
    try {
      ...
      try {
        browserState.context = await playwright.chromium.launchPersistentContext(
          userDataDir,
          launchOpts,
        );
      } catch (err) {
        // Channel (e.g. "chrome") not installed — retry with bundled chromium.
      ...
      browserState.browser = browserState.context.browser?.() || null;
      const pages = browserState.context.pages();
      browserState.page = pages.length ? pages[0] : await browserState.context.newPage();
    } catch (err) {
      throw new Error(
        `Failed to launch persistent browser at ${userDataDir}: ${err.message}. ` +
          `If this is the user's real Chrome profile, close Chrome first or pick a dedicated profile.`,
      );
    }
...
// ─── Active-session registry & cancel/redirect control ─────────────────────

function registerActiveSession(id, info) {
  try {
    const dir = systemExecuteActiveDir();
    fs.mkdirSync(dir, { recursive: true });
    fs.writeFileSync(
      path.join(dir, `${id}.json`),
      JSON.stringify({ id, ...info }, null, 2),
    );
  } catch {}
}

function unregisterActiveSession(id) {
  try {
    fs.unlinkSync(path.join(systemExecuteActiveDir(), `${id}.json`));
  } catch {}
}

// Reads the control file once. If a directive (cancel/redirect) targets
// this session and was issued after sessionStartedAt, returns the
// directive and deletes the file. Otherwise returns null.
function consumeControlDirective(sessionId, sessionStartedAt) {
  const file = systemExecuteControlPath();
  let raw;
  try {
  ...
  // Stale directive (from before this session started) — drop.
  const ts = Number(parsed?.ts || 0);
  if (!Number.isFinite(ts) || ts < sessionStartedAt) {
    return null;
  }
  // Targeted directive must match this session.
  if (parsed.targetSession && parsed.targetSession !== sessionId) {
    return null;
  }
  try { fs.unlinkSync(file); } catch {}
  return parsed;
...
async function handleBuildWorkspaceTool(args, _deps) {
  const toolRequest = String(args?.tool_request || "").trim();
  if (!toolRequest) {
    return [{ type: "text", text: "build_workspace_tool: 'tool_request' is required." }];
  }

  const hint = args?.tool_name_hint ? String(args.tool_name_hint).trim() : null;
  const lang = args?.language === "shell" ? "shell" : "javascript";
  const nameLine = hint
    ? `Name it: ${hint} (or a close variant in snake_case).`
    : "Choose a short, descriptive snake_case name.";
  ...
  return [
    {
      ...
      text: [
        ...
        "",
        "Steps:",
        `1. Write the ${lang} implementation below.`,
        `   ${lang === "javascript"
          ? "JS contract: module.exports = async (args) => { return 'result string'; }"
          : "Shell contract: read args from $GSH_TOOL_ARGS (JSON string), write result to stdout."}`,
        `2. ${nameLine}`,
        "3. Write a one-sentence description of what the tool does.",
        "4. Call register_workspace_tool with name, description, script, and language.",
        "   The tool is immediately callable via tools/call — no restart needed.",
        "",
        "Write the implementation now, then call register_workspace_tool.",
      ].join("\n"),
    },
  ];
}
...
async function handleSystemExecute(args, deps) {
  ...
  const sessionId = `${process.pid}-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
  const sessionStartedAt = Date.now();
  registerActiveSession(sessionId, { task, model, pid: process.pid, startedAt: sessionStartedAt });
  ...
  const systemPrompt = [
    ...
    "6. The user can interrupt you at any time by pressing Cmd+Shift+Esc. If you receive a `[USER INTERRUPT]` message mid-loop, treat its content as a course correction and resume from your current state.",
    "",
  ...
  try {
    for (let i = 0; i < maxIter; i += 1) {
      ...
      // Honor user interrupts (Cmd+Shift+Esc) before every iteration.
      const directive = consumeControlDirective(sessionId, sessionStartedAt);
      if (directive) {
        if (directive.cancel || (directive.redirect != null && !String(directive.redirect).trim())) {
          stopReason = "canceled_by_user";
          finalAnswer = "[canceled by user]";
          break;
        }
        if (directive.redirect) {
          messages.push({
            role: "user",
            content: `[USER INTERRUPT]\n${String(directive.redirect)}\n\nIncorporate this and continue from your current state.`,
          });
          transcript.push({
            iteration: iterations,
            tool: "__user_interrupt__",
            args: {},
            result: truncate(String(directive.redirect), 400),
          });
        }
      }
...
function createLocalSubagentHandler(deps) {
  return async function handleLocalSubagentTool(toolName, toolArguments) {
    ...
    if (toolName === "build_workspace_tool") {
      return handleBuildWorkspaceTool(toolArguments || {}, deps || {});
    }
    return null;
  };
}
