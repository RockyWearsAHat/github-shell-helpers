...
async function handleListLanguageModels(options = {}) {
  ...
  try {
    ...
    const lines = [
      ...
      ...(data.models || []).map(
        (model) =>
          `| ${escapeMarkdownTableCell(model.id)} | ${escapeMarkdownTableCell(model.name)} | ${escapeMarkdownTableCell(model.vendor)} | ${escapeMarkdownTableCell(model.qualifiedName)} | ${escapeMarkdownTableCell(model.maxInputTokens)} |`,
      ),
      "",
