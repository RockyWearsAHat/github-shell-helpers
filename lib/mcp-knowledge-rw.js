...
module.exports = function createKnowledgeRW(deps) {
  ...
  let buildKnowledgeIndex = deps.buildKnowledgeIndex || null;
  let buildKnowledgeSnippet = deps.buildKnowledgeSnippet || null;
  let scoreKnowledgeMatch = deps.scoreKnowledgeMatch || null;
  let collectMarkdownFiles = deps.collectMarkdownFiles || null;
  ...
  function attachKnowledgeIndex(indexFns = {}) {
    if (typeof indexFns.buildKnowledgeIndex === "function") {
      buildKnowledgeIndex = indexFns.buildKnowledgeIndex;
    }
    if (typeof indexFns.buildKnowledgeSnippet === "function") {
      buildKnowledgeSnippet = indexFns.buildKnowledgeSnippet;
    }
    if (typeof indexFns.scoreKnowledgeMatch === "function") {
      scoreKnowledgeMatch = indexFns.scoreKnowledgeMatch;
    }
    if (typeof indexFns.collectMarkdownFiles === "function") {
      collectMarkdownFiles = indexFns.collectMarkdownFiles;
    }
  }

  function requireKnowledgeIndexBinding(name, value) {
    if (typeof value === "function") {
      return value;
    }
    throw new Error(`Knowledge index binding '${name}' is not attached yet.`);
  }

  async function rebuildLocalKnowledgeIndex() {
    try {
      const result = await requireKnowledgeIndexBinding(
        "buildKnowledgeIndex",
        buildKnowledgeIndex,
      )({});
      return {
  ...
  async function searchKnowledgeCache(args) {
    ...
    const files = [];
    const collectFiles = requireKnowledgeIndexBinding(
    ...
    const results = [];
    const matchScore = requireKnowledgeIndexBinding(
      "scoreKnowledgeMatch",
      scoreKnowledgeMatch,
    );
    const buildSnippet = requireKnowledgeIndexBinding(
    ...
    for (const { filePath: fp, root } of files) {
      ...
      const score = matchScore(relativePath, title, body, terms);
      if (score <= 0) {
        continue;
      }
